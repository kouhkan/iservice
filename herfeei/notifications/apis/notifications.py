...
class GetUserNotificationsView(ApiAuthMixin, APIView):
    class NotificationFilterSerializer(serializers.Serializer):
        category = serializers.SlugField(min_length=1, max_length=64, required=False)
    ...
    def get(self, request):
        filter_serializer = self.NotificationFilterSerializer(data=request.query_params)
        filter_serializer.is_valid(raise_exception=True)
        queryset = get_user_notifications(user=request.user)
        queryset = self.apply_filter(queryset, filter_serializer.validated_data)
        return Response(
