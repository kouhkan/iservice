...
def get_most_rated_experts(
        *, service_category_slug: str) -> List[QuerySet[Expert]]:
    return (Expert.objects.filter(
        comments__status=Comment.CommentStatus.APPROVE,
        category__slug=service_category_slug).values(
            'category__title').annotate(
                avg_rating=Avg('comments__rate'),
                num_ratings=Count('comments')).order_by(
