...
from herfeei.core.exceptions import OrderTimeException, CompleteEmailProfileException, OwnerOfOrderException
...
class UserOrderView(ApiAuthMixin, APIView):
    class InputOrderSerializer(serializers.ModelSerializer):
        class Meta:
            model = Order
            fields = (
                "user_answer", "description", "for_other", "discount", "expert",
                "service", "order_date_time", "now", "email_order", "user_address"
            )
        ...
        def validate(self, attrs):
            ...
            if attrs.get("email_order"):
                if not attrs.get("user_answer").user.profile.email:
                    raise CompleteEmailProfileException("User must set email in profile")

            if self.context.get("request").user != attrs.get("user_answer").user:
                raise OwnerOfOrderException("This order is not own of this user")
    ...
    @extend_schema(request=InputOrderSerializer, responses=OutputOrderSerializer)
    def post(self, request):
        serializer = self.InputOrderSerializer(data=request.data, context={"request": request})
        ...
        return Response(self.OutputOrderSerializer(order).data, status=status.HTTP_201_CREATED)
