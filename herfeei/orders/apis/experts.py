...
from herfeei.orders.selectors.experts import get_bookmarked_experts, get_most_rated_experts
...
class BookmarkExpertsView(ApiAuthMixin, APIView):
    ...
    def get(self, request):
        return Response(self.OutputBookmarkExpertsSerializer(get_bookmarked_experts(user=request.user), many=True).data)
...
class MostRatedExpertsView(ApiAuthMixin, APIView):
    ...
    def get(self, request, category_slug):
        return Response(
            self.OutputMostRatedExpertsView(get_most_rated_experts(service_category_slug=category_slug), many=True).data
        )
