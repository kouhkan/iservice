...
def get_expert_rate(*, expert_code: str) -> float:
    expert_rate = Expert.objects.filter(expert_code=expert_code).annotate(
        avg_rate=Avg('comments__rate', filter=Q(comments__status=Comment.CommentStatus.APPROVE)),
        comment_count=Count('comments', filter=Q(comments__status=Comment.CommentStatus.APPROVE))
    ).first()
...
def get_expert_comments_count(*, expert_code: str) -> int:
    return Comment.objects.filter(
        expert__expert_code=expert_code, status=Comment.CommentStatus.APPROVE
    ).count()


def get_expert_comments(*, expert_code: str) -> int:
    return Comment.objects.filter(expert__expert_code=expert_code, status=Comment.CommentStatus.APPROVE)


def get_expert_complete_orders(*, expert_code: str) -> int:
    return Order.objects.filter(expert__expert_code=expert_code, is_complete=True).count()
