...
class LoginByUsernameView(APIView):
    ...
    def post(self, request):
        ...
        if wait_time := get_auth_prepration_wait_time(
                serializer.validated_data.get("username")):
            return Response(f"{wait_time} second(s) left to get a new token",
                            status=status.HTTP_429_TOO_MANY_REQUESTS)

        token = prepare_authentication_token(
            auth_key=serializer.validated_data.get("username"))
...
class VerifyAuthenticationView(APIView):
    ...
    @extend_schema(request=InputVerifyAuthenticationSerializer,
                   responses=OutputVerifyAuthenticationSerializer)
    def post(self, request):
        serializer = self.InputVerifyAuthenticationSerializer(
            data=request.data)
        serializer.is_valid(raise_exception=True)

        if not check_authentication_token(
                auth_key=serializer.validated_data.get("username"),
        ...
        try:
            ...
            return Response(f"Database Error {ex}",
                            status=status.HTTP_400_BAD_REQUEST)
        return Response(
            self.OutputVerifyAuthenticationSerializer(user,
                                                      context={
                                                          "request": request
                                                      }).data)
